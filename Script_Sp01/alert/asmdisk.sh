...
if [ $size -lt 500 ]; then
echo "ASM Disk Group " $disk "Member" $name "out of size "  $size "MB"  >>  $HOME/alert/logs/asmdisk_alert_$DT
/bin/mailx -s "ASM DISK SPACE USAGE by PWC :FOR $ORACLE_UNQNAME" caesar.dutta@in.pwc.com  < $HOME/alert/logs/asmdisk_alert_$DT
#size=$(echo $output | awk '{print $1}' | cut -d  " " -f1 )
for ((i=0;i<1;i++))
  do
( echo open 192.168.22.110 25
...
echo mail from: $HOSTNAME
sleep 2
echo rcpt to: caesar.dutta@in.pwc.com
sleep 2
echo data
sleep 2
echo subject: ASM DISK SPACE USAGE ALERT FOR $ORACLE_SID
...
if [ $size -lt 500 ]; then
echo "ASM Disk Group " $disk "Member" $name "out of size "  $size "MB"  >>  $HOME/alert/logs/asmdisk_alert_$DT
/bin/mailx -s "ASM DISK SPACE USAGE by PWC :FOR $ORACLE_UNQNAME" ritayan.banerjee@srei.com  < $HOME/alert/logs/asmdisk_alert_$DT
#size=$(echo $output | awk '{print $1}' | cut -d  " " -f1 )
fi

if [ $size -lt 500 ]; then
echo "ASM Disk Group " $disk "Member" $name "out of size "  $size "MB"  >>  $HOME/alert/logs/asmdisk_alert_$DT
/bin/mailx -s "ASM DISK SPACE USAGE by PWC :FOR $ORACLE_UNQNAME" yogeshk@srei.com,abhijit.chakraborty@in.pwc.com  < $HOME/alert/logs/asmdisk_alert_$DT
#size=$(echo $output | awk '{print $1}' | cut -d  " " -f1 )
fi
done
