  ...
  if [ $cpu -lt 20 ]
             then
               ...
               ##mutt -s "Warning Database running slow $ORACLE_SID" db.support@srei.com
                echo wget --no-check-certificate "http://mobiprom.com/smsclient/pushsms.jsp?user=CL20640&password=XGFz5Hw&message=I/O WAIT is above 90% in instance $HOSTNAME. Thank You&from=SreiBP&to=9830415775" >> dbcpu_$ORACLE_SID
		echo wget --no-check-certificate "http://mobiprom.com/smsclient/pushsms.jsp?user=CL20640&password=XGFz5Hw&message=I/O WAIT is above 90% in instance $HOSTNAME. Thank You&from=SreiBP&to=08983600103"  >> dbcpu_$ORACLE_SID
		echo wget --no-check-certificate "http://mobiprom.com/smsclient/pushsms.jsp?user=CL20640&password=XGFz5Hw&message=I/O WAIT is above 90% in instance $HOSTNAME. Thank You&from=SreiBP&to=09021127754"  >> dbcpu_$ORACLE_SID
             fi
...
cp dbcpu_$ORACLE_SID /newdump/filesystems/sms/
