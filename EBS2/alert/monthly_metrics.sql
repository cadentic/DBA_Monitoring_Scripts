...
BEGIN
	...
	BEGIN
            ...
            AND   INST_ID = 2;
                ...
                IF v_prev_day_count > 1 THEN
                  ...
                  AND         INST_ID = 2;
                        ...
                        DELETE FROM TBL_MONTHLY_CPU_METRICS  WHERE DAY = TO_CHAR(TRUNC(SYSDATE-1,'DDD'),'DD-MON-YYYY') AND INST_ID = 2;
                ...
                VALUES (2, TO_CHAR(SYSDATE-1, 'MONTH'), TO_CHAR(TRUNC(SYSDATE-1,'DDD'),'DD-MON-YYYY'), v_host_cpu_util, v_ora_cpu_util, v_ora_cpu_wait,
      ...
      and    inst_id = 2;
        ...
        VALUES (2, v_month, v_day, v_host_cpu_util, v_ora_cpu_util, v_ora_cpu_wait, v_host_cpu_util, v_ora_cpu_util, v_ora_cpu_wait,SYSDATE );
...
BEGIN
            ...
            AND   INST_ID = 2;
                ...
                IF v_prev_day_count > 1 THEN
                  ...
                  AND         INST_ID = 2;
                        ...
                        -- Delete all previous day record
                        DELETE FROM TBL_MONTHLY_MEM_METRICS  WHERE DAY = TO_CHAR(TRUNC(SYSDATE-1,'DDD'),'DD-MON-YYYY') AND INST_ID = 2;
                    ...
                    PHYSICAL_MEMORY,  MAX_SGA_TARGET, MAX_PGA_AGGREGATE_TARGET, MAX_PGA_ALLOCATED,  MAX_PGA_USED, MAX_SGA_VARIABLE_SIZE, TO_DAY
                        ...
                        2, TO_CHAR(SYSDATE-1, 'MONTH'), TO_CHAR(TRUNC(SYSDATE-1,'DDD'),'DD-MON-YYYY'), v_sga_max_size, v_sga_var, v_sga_target,
        ...
        INTO    v_sga_max_size
        FROM    GV$SGAINFO  WHERE NAME = 'Maximum SGA Size' AND INST_ID=2;


        SELECT  ROUND(SUM(BYTES)/1024/1024/1024,2)
        INTO    v_sga_var
        FROM    GV$SGAINFO WHERE        RESIZEABLE='Yes'  AND INST_ID = 2;


        SELECT  ROUND(TO_NUMBER(VALUE)/1024/1024/1024,2)
        INTO    v_sga_target
        FROM    GV$PARAMETER    WHERE UPPER(NAME)='SGA_TARGET'  AND INST_ID = 2;
        ...
        FROM
                (select value  tot_pag_used
                 from   gv$pgastat where name = 'total PGA used for auto workareas' and inst_id = 2),
        ...
        INTO    v_physical_ram
        FROM    GV$OSSTAT WHERE STAT_NAME = 'PHYSICAL_MEMORY_BYTES' AND INST_ID = 2;
        ...
        INSERT INTO TBL_MONTHLY_MEM_METRICS (
          ...
          2, v_month, v_day, v_sga_max_size, v_sga_var, v_sga_target,  v_pga_target,  v_pga_alloc,  v_pga_used, v_physical_ram ,
